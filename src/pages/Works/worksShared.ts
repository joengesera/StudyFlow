...
export function getScoreColor(earned: number | null | undefined, possible: number | null | undefined) {
  if (earned == null || possible == null) return 'text-on-surface-variant';
...
export function getStatusBadge(workStatus: WorkStatus) {
  if (workStatus === 'GRADED') {
    return { bg: 'bg-primary/10 text-primary', label: 'Noté' };
  }
  if (workStatus === 'SUBMITTED') {
    return { bg: 'bg-tertiary/10 text-tertiary', label: 'Soumis' };
  }
  return { bg: 'bg-surface-container-highest text-on-surface-variant', label: 'Planifié' };
}