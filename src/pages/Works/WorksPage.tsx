...
export default function WorksPage() {
  ...
  const filteredWorks = useMemo(() => filterAndSortWorks(works, statusFilter, courseFilter), [works, statusFilter, courseFilter]);

  const closeModal = () => { setSelectedWork(null); setIsCreating(false); };

  const handleSave = (form: Partial<Work>) => {
    if (selectedWork) updateWork({ id: selectedWork.id, payload: form });
    else createWork(form);
    closeModal();
  };

  const handleDelete = (id: string) => { deleteWork(id); closeModal(); };

  if (isLoading) {
    return <div className="flex flex-col items-center justify-center h-[400px] text-on-surface-variant">Chargement des travaux...</div>;
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {(selectedWork || isCreating) && (
        <WorkModal work={selectedWork} courses={courses} onClose={closeModal} onSave={handleSave} onDelete={handleDelete} />
      )}
    </div>
  );
}